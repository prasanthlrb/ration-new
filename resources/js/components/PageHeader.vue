<template>
    <div class="d-flex mb-3 align-items-center">
        <div class="flex-grow-1 d-flex align-items-center">
            ...
            <h4 class="flex-grow-1 page-title fw-medium text-capitalize m-0 p-0"> {{ title }} <span v-if="subTitle"
                    class="font-15 text-muted">{{ subTitle }}</span></h4>
        </div>
        <div>
            <template v-if="getItems.length > 0">
                <ol class="breadcrumb m-0">
                    <li v-for="(item, index) in getItems" :key="index" :class="{ active: item.active }"
